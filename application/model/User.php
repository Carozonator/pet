...
class User extends Model{
    ...
    function getPublicaciones(){
        ...
        foreach($tables as $table){
            $sql =    "SELECT $table.*, foto.name as foto_name, foto.usuario as foto_usuario FROM user "
                    ...
                    . "LEFT OUTER JOIN (SELECT * FROM foto where _table='$table' and foto.usuario=? order by photo_order) as foto on foto.publication_id= $table.id "
                    . "WHERE user.id=? "
    ...
    function getPublicacionesWithQuestions(){
        ...
        $results=array();
        foreach($tables as $table){
            $sql =    "SELECT pregunta.id as pregunta_id,pregunta.question, pregunta.question_timestamp, $table.*, foto.name as foto_name, foto.usuario as foto_usuario "
                    . "FROM user "
                    . "INNER JOIN $table on user.id=$table.usuario "
                    . "LEFT OUTER JOIN foto on foto.publication_id= $table.id "
                    . "LEFT OUTER JOIN pregunta on pregunta.publication_id= $table.id "
                    . "WHERE user.id=? and pregunta.respondent=? and pregunta.answer is null and pregunta.question is not null "
